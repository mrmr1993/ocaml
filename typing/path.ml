...
let rec find_free_opt ids = function
    Pident id ->
      begin match List.find_opt (Ident.same id) ids with
      | Some _ as res -> res
      | None ->
          if Ident.is_instantiable id then
            Option.bind (Ident.instantiation id) (find_free_opt ids)
          else None
      end
...
let rec subst_aux find = function
    Pident id when Ident.is_instantiable id ->
      begin match Ident.instantiation id with
      | Some path ->
          begin try Pident (find id) with Not_found -> subst_aux find path end
      | None -> Pident (find id)
      end
...
let rec print ppf = function
  | Pident id when Ident.is_instantiable id ->
      begin match Ident.instantiation id with
      | Some path -> print ppf path
      | None -> Ident.print_with_scope ppf id
      end
...
let heads p =
  let rec heads p acc = match repr p with
    | Pident id -> id :: acc
