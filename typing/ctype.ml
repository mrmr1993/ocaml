...
let rec copy_sep cleanup_scope fixed free bound visited id_pairs ty =
  ...
  if TypeSet.is_empty univars then
    ...
    let t = newvar () in
    delayed_copy :=
      lazy (t.desc <- Tlink (copy' cleanup_scope id_pairs ty))
      :: !delayed_copy;
    t
  else try
...
let rec local_non_recursive_abbrev strict visited env id_pairs p ty =
    ...
    match ty.desc with
      Tconstr(p', args, _abbrev) ->
        ...
        begin try
          ...
          local_non_recursive_abbrev strict visited env id_pairs p
        ...
        with Cannot_expand ->
          ...
          List.iter2
            (fun tv ty ->
              ...
              local_non_recursive_abbrev strict visited env id_pairs p ty)
            params args
        end
    | _ ->
        if strict then (* PR#7374 *)
          let visited = ty :: visited in
          iter_type_expr
            (local_non_recursive_abbrev true visited env id_pairs p)
            ty
  end
...
let occur_univar env id_pairs ty =
...
and unify3 env id_pairs1 id_pairs2 t1 t1' t2 t2' =
  ...
  | (_, Tvar _) ->
      ...
      (* TODO: Check that [t1] doesn't contain any unscoped identifiers. *)
      occur_univar !env [] t1;
      link_type t2' t1;
  ...
  | _ ->
    ...
    try
      ...
      | (Tconstr (_,_,_), _) | (_, Tconstr (_,_,_)) when !umode = Pattern ->
          ...
          if !generate_equations then mcomp !env id_pairs1 id_pairs2 t1' t2'
...
and unify_row_field env id_pairs1 id_pairs2 fixed1 fixed2 rm1 rm2 l f1 f2 =
  ...
  | Reither(c1, tl1, m1, e1), Reither(c2, tl2, m2, e2) ->
      ...
      let split_univars id_pairs =
        List.partition
          (fun ty ->
            try occur_univar !env id_pairs ty; true with Unify _ -> false)
      in
    ...
    ty'.desc <-
      ...
      | Tconstr(p, tl, _abbrev) ->
          ...
          | Some id ->
              begin try
                Tlink
                  (nondep_type_rec ~expand_private env id_pairs ids
