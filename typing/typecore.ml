...
and type_expect_
  ...
  | Pexp_functor (name, (p, l), sbody) ->
      ...
      let pack, pack_ty =
        ...
        in
        let path =
          !Typetexp.transl_modtype_longident p.loc env p.txt
        in
        let ty =
          ( path
      ...
      set_type_module_scope (Ctype.get_current_level ()) name.txt;
      let new_env = Env.add_module name.txt Mp_present mty.mty_type env in
      let body = type_expect new_env sbody (mk_expected (newvar ())) in
      end_def();
      let exp_type = newty (Tfunctor (name.txt, pack_ty, body.exp_type)) in
      generalize exp_type;
      set_type_module_scope original_name_scope name.txt;
      unify_var new_env (newvar()) exp_type;
      rue {
        exp_desc = Texp_functor(name, pack, body);
        exp_loc = loc; exp_extra = [];
        exp_type = exp_type;
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
