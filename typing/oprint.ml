...
and print_out_type_1 ppf =
  function
    Otyp_arrow (lab, ty1, ty2) ->
      ...
      pp_close_box ppf ()
  | Otyp_module_arrow (lab, p, n, tyl, ty) ->
      pp_open_box ppf 0;
      fprintf ppf "@[<1>{%a : %a}@] ->@ %a"
        print_lident lab.printed_name
        print_package_type (p, n, tyl)
        print_out_type_1 ty ;
      pp_close_box ppf ()
...
and print_simple_out_type ppf =
  ...
  | Otyp_variant (non_gen, row_fields, closed, tags) ->
      ...
      fprintf ppf "%s@[<hov>[%s@[<hv>@[<hv>%a@]%a@]@ ]@]"
        ...
        print_present tags
  | Otyp_alias _ | Otyp_poly _ | Otyp_arrow _ | Otyp_tuple _
  | Otyp_module_arrow _ as ty ->
      pp_open_box ppf 1;
      pp_print_char ppf '(';
  ...
  | Otyp_module (p, n, tyl) ->
      fprintf ppf "@[<1>(module %a)@]" print_package_type (p, n, tyl)
